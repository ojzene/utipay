...
/**
 * Class for receiving or sending sms through SMPPX protocol.
 * This is a reduced implementation of the SMPPX protocol, and as such not all features will or ought to be available.
 * The purpose is to create a lightweight and simplified SMPPX client.
 * 
 * @author hd@onlinecity.dk, paladin
 * @see http://en.wikipedia.org/wiki/Short_message_peer-to-peer_protocol - SMPPX 3.4 protocol specification
...
class SmppClient
{
	...
	/**
	 * SMPPX v3.4 says octect string are "not necessarily NULL terminated".
	...
	public function bindReceiver($login, $pass)
	{
		...
		$response = $this->_bind($login, $pass, SMPPX::BIND_RECEIVER);
	...
	public function bindTransmitter($login, $pass)
	{
		...
		$response = $this->_bind($login, $pass, SMPPX::BIND_TRANSMITTER);
	...
	public function close()
	{
		...
		$response=$this->sendCommand(SMPPX::UNBIND,"");
	...
	/**
	 * Parse a timestring as formatted by SMPPX v3.4 section 7.1.
	 ...
	 * 	message_state would be one of the SMPPX::STATE_* constants. (SMPPX v3.4 section 5.2.28)
	...
	public function queryStatus($messageid,SmppAddress $source)
	{
		...
		$reply = $this->sendCommand(SMPPX::QUERY_SM, $pduBody);
		if (!$reply || $reply->status != SMPPX::ESME_ROK) return null;
	...
	public function readSMS()
	{
		...
		do{
			...
			if($pdu->id==SMPPX::ENQUIRE_LINK) {
				$response = new SmppPdu(SMPPX::ENQUIRE_LINK_RESP, SMPPX::ESME_ROK, $pdu->sequence, "\x00");
	 ...
	 */
	public function sendSMS(SmppAddress $from, SmppAddress $to, $message, $tags=null, $dataCoding=SMPPX::DATA_CODING_DEFAULT, $priority=0x00, $scheduleDeliveryTime=null, $validityPeriod=null)
	{
		$msg_length = strlen($message);
		
		if ($msg_length>160 && $dataCoding != SMPPX::DATA_CODING_UCS2 && $dataCoding != SMPPX::DATA_CODING_DEFAULT) return false;
		
		switch ($dataCoding) {
			case SMPPX::DATA_CODING_UCS2:
				...
				break;
			case SMPPX::DATA_CODING_DEFAULT:
				...
				break;
			default:
				$singleSmsOctetLimit = 254; // From SMPPX standard
				break;
		}
	 ...
	 * @return string message id
	 */
	protected function submit_sm(SmppAddress $source, SmppAddress $destination, $short_message=null, $tags=null, $dataCoding=SMPPX::DATA_CODING_DEFAULT, $priority=0x00, $scheduleDeliveryTime=null, $validityPeriod=null, $esmClass=null)
	{
		...
		$response=$this->sendCommand(SMPPX::SUBMIT_SM,$pdu);
	 ...
	 * @param integer $dataCoding (optional)
	 */
	protected function splitMessageString($message, $split, $dataCoding=SMPPX::DATA_CODING_DEFAULT)
	{
		switch ($dataCoding) {
			case SMPPX::DATA_CODING_DEFAULT:
				$msg_length = strlen($message);
				...
				$parts[] = $part;
				return $parts;
			case SMPPX::DATA_CODING_UCS2: // UCS2-BE can just use str_split since we send 132 octets per message, which gives a fine split using UCS2
			default:
	...
	protected function _bind($login, $pass, $command_id)
	{
		...
		if ($response->status != SMPPX::ESME_ROK) throw new SmppException(SMPPX::getStatusMessage($response->status), $response->status);
	...
	{
		// Check command id
		if($pdu->id != SMPPX::DELIVER_SM) throw new InvalidArgumentException('PDU is not an received SMS');
		...
		if (($esmClass & SMPPX::ESM_DELIVER_SMSC_RECEIPT) != 0) {
		...
		$response = new SmppPdu(SMPPX::DELIVER_SM_RESP, SMPPX::ESME_ROK, $pdu->sequence, "\x00");
		$this->sendPDU($response);
		return $sms;
	}
	...
	{
		$response = $this->sendCommand(SMPPX::ENQUIRE_LINK, null);
		return $response;
	}
	...
	public function respondEnquireLink()
	{
		...
		for($i=0;$i<$ql;$i++) {
			...
			if($pdu->id==SMPPX::ENQUIRE_LINK) {
				//remove response
				array_splice($this->pdu_queue, $i, 1);
				$this->sendPDU(new SmppPdu(SMPPX::ENQUIRE_LINK_RESP, SMPPX::ESME_ROK, $pdu->sequence, "\x00"));
			}
		}
	 ...
	 * Some SMPPX servers, ie. logica returns GENERIC_NACK on errors.
	 * 
	...
	protected function readPDU_resp($seq_number, $command_id)
	{
		...
		$command_id=$command_id|SMPPX::GENERIC_NACK;
		...
		for($i=0;$i<$ql;$i++) {
			$pdu=$this->pdu_queue[$i];
			if (
				($pdu->sequence == $seq_number && ($pdu->id == $command_id || $pdu->id == SMPPX::GENERIC_NACK)) ||
				($pdu->sequence == null && $pdu->id == SMPPX::GENERIC_NACK)
			) {
		...
		do{
			$pdu=$this->readPDU();
			if ($pdu) {
				if ($pdu->sequence == $seq_number && ($pdu->id == $command_id || $pdu->id == SMPPX::GENERIC_NACK)) return $pdu;
				if ($pdu->sequence == null && $pdu->id == SMPPX::GENERIC_NACK) return $pdu;
	...
	protected function readPDU()
	{
		...
		if($this->debug) {
			...
			call_user_func($this->debugHandler, " command status  : 0x".dechex($command_status)." ".SMPPX::getStatusMessage($command_status));
...
/**
 * Numerous constants for SMPPX v3.4
 * Based on specification at: http://www.smsforum.net/SMPP_v3_4_Issue1_2.zip
 */ 

class SMPP
{
	// Command ids - SMPPX v3.4 - 5.1.2.1 page 110-111
	...
	//  Command status - SMPPX v3.4 - 5.1.3 page 112-114
	...
	// ESM bits 1-0 - SMPPX v3.4 - 5.2.12 page 121-122
	...
	// SMPPX v3.4 - 5.2.18 page 125
	const REPLACE_NO = 0x00;
	const REPLACE_YES = 0x01;
	
	// SMPPX v3.4 - 5.2.19 page 126
	...
	// SMPPX v3.4 - 5.2.25 page 129
	const DEST_FLAG_SME = 1;
	const DEST_FLAG_DISTLIST = 2;
	
	// SMPPX v3.4 - 5.2.28 page 130
	...
	{
		switch ($statuscode) {
			case SMPPX::ESME_ROK: return 'No Error';
			case SMPPX::ESME_RINVMSGLEN: return 'Message Length is invalid';
			case SMPPX::ESME_RINVCMDLEN: return 'Command Length is invalid';
			case SMPPX::ESME_RINVCMDID: return 'Invalid Command ID';
			case SMPPX::ESME_RINVBNDSTS: return 'Incorrect BIND Status for given command';
			case SMPPX::ESME_RALYBND: return 'ESME Already in Bound State';
			case SMPPX::ESME_RINVPRTFLG: return 'Invalid Priority Flag';
			case SMPPX::ESME_RINVREGDLVFLG: return 'Invalid Registered Delivery Flag';
			case SMPPX::ESME_RSYSERR: return 'System Error';
			case SMPPX::ESME_RINVSRCADR: return 'Invalid Source Address';
			case SMPPX::ESME_RINVDSTADR: return 'Invalid Dest Addr';
			case SMPPX::ESME_RINVMSGID: return 'Message ID is invalid';
			case SMPPX::ESME_RBINDFAIL: return 'Bind Failed';
			case SMPPX::ESME_RINVPASWD: return 'Invalid Password';
			case SMPPX::ESME_RINVSYSID: return 'Invalid System ID';
			case SMPPX::ESME_RCANCELFAIL: return 'Cancel SM Failed';
			case SMPPX::ESME_RREPLACEFAIL: return 'Replace SM Failed';
			case SMPPX::ESME_RMSGQFUL: return 'Message Queue Full';
			case SMPPX::ESME_RINVSERTYP: return 'Invalid Service Type';
			case SMPPX::ESME_RINVNUMDESTS: return 'Invalid number of destinations';
			case SMPPX::ESME_RINVDLNAME: return 'Invalid Distribution List name';
			case SMPPX::ESME_RINVDESTFLAG: return 'Destination flag (submit_multi)';
			case SMPPX::ESME_RINVSUBREP: return 'Invalid ‘submit with replace’ request (i.e. submit_sm with replace_if_present_flag set)';
			case SMPPX::ESME_RINVESMSUBMIT: return 'Invalid esm_SUBMIT field data';
			case SMPPX::ESME_RCNTSUBDL: return 'Cannot Submit to Distribution List';
			case SMPPX::ESME_RSUBMITFAIL: return 'submit_sm or submit_multi failed';
			case SMPPX::ESME_RINVSRCTON: return 'Invalid Source address TON';
			case SMPPX::ESME_RINVSRCNPI: return 'Invalid Source address NPI';
			case SMPPX::ESME_RINVDSTTON: return 'Invalid Destination address TON';
			case SMPPX::ESME_RINVDSTNPI: return 'Invalid Destination address NPI';
			case SMPPX::ESME_RINVSYSTYP: return 'Invalid system_type field';
			case SMPPX::ESME_RINVREPFLAG: return 'Invalid replace_if_present flag';
			case SMPPX::ESME_RINVNUMMSGS: return 'Invalid number of messages';
			case SMPPX::ESME_RTHROTTLED: return 'Throttling error (ESME has exceeded allowed message limits)';
			case SMPPX::ESME_RINVSCHED: return 'Invalid Scheduled Delivery Time';
			case SMPPX::ESME_RINVEXPIRY: return 'Invalid message (Expiry time)';
			case SMPPX::ESME_RINVDFTMSGID: return 'Predefined Message Invalid or Not Found';
			case SMPPX::ESME_RX_T_APPN: return 'ESME Receiver Temporary App Error Code';
			case SMPPX::ESME_RX_P_APPN: return 'ESME Receiver Permanent App Error Code';
			case SMPPX::ESME_RX_R_APPN: return 'ESME Receiver Reject Message Error Code';
			case SMPPX::ESME_RQUERYFAIL: return 'query_sm request failed';
			case SMPPX::ESME_RINVOPTPARSTREAM: return 'Error in the optional part of the PDU Body.';
			case SMPPX::ESME_ROPTPARNOTALLWD: return 'Optional Parameter not allowed';
			case SMPPX::ESME_RINVPARLEN: return 'Invalid Parameter Length.';
			case SMPPX::ESME_RMISSINGOPTPARAM: return 'Expected Optional Parameter missing';
			case SMPPX::ESME_RINVOPTPARAMVAL: return 'Invalid Optional Parameter Value';
			case SMPPX::ESME_RDELIVERYFAILURE: return 'Delivery Failure (data_sm_resp)';
			case SMPPX::ESME_RUNKNOWNERR: return 'Unknown Error';
			default:
...
class SmppDeliveryReceipt extends SmppSms
{
	...
	/**
	 * Parse a delivery receipt formatted as specified in SMPPX v3.4 - Appendix B
...
class SmppAddress
{
	 ...
	 * @throws InvalidArgumentException
	 */
	public function __construct($value, $ton=SMPPX::TON_UNKNOWN, $npi=SMPPX::NPI_UNKNOWN)
	{
		// Address-Value field may contain 10 octets (12-length-type), see 3GPP TS 23.040 v 9.3.0 - section 9.1.2.5 page 46.
		if ($ton == SMPPX::TON_ALPHANUMERIC && strlen($value) > 11) throw new InvalidArgumentException('Alphanumeric address may only contain 11 chars');
		if ($ton == SMPPX::TON_INTERNATIONAL && $npi == SMPPX::NPI_E164 && strlen($value) > 15) throw new InvalidArgumentException('E164 address may only contain 15 digits');
...
/**
 * Primitive class to represent SMPPX optional params, also know as TLV (Tag-Length-Value) params
 * @author hd@onlinecity.dk
 */
class SmppTag
{
