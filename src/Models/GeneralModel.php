...
class GeneralModel
{
    ...
    public function try_get($req_res, $model_method_array, $i, $output_format, $input=null, $params=null)
    {
        try{
            ...
            if ($output_data["status"] == "success"  || $output_data["status"] == true) { $httpstatus = 200; } else { $httpstatus = 400; }
//          if ($output_data["status"] == "success" || $output_data['success'] == "true"  || $output_data["status"] == true || $output_data['success'] == true) { $httpstatus = 200; } else { $httpstatus = 400; }
...
//            $result = [ "success" => false, "status" => false, "message" => $status['data'], "code" => $status['code'] ];
